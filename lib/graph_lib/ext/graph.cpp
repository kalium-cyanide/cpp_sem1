...
Box::Box(Point xy, int ww, int hh, int radius)
  ...
  if (r < 0) error("Bad box: negative round radius");

  if (r == automatic) r = int(0.1 * std::min(ww, hh));
}

Box::Box(Point x, Point y, int radius) : Rectangle{x, y}, r{radius} {
  if (r < 0) error("Bad box: negative round radius");

  if (r == automatic) r = int(0.1 * std::min(width(), height()));
}
...
void Hexagon_tile::move(int dx, int dy) {
  ...
  for (int i = 0; i < tile.size(); ++i) tile[i].move(dx, dy);
}

void Hexagon_tile::draw_lines() const {
  Rectangle::draw_lines();
  for (int i = 0; i < tile.size(); ++i) tile[i].draw();
}
