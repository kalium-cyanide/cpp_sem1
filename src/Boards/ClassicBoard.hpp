...
class ClassicBoard : public IBoard
{
  ...
  public:
    ...
    IFigure* getFigure(position_t position) override { return state.figures_m[position.x - 1][position.y - 1]; }
    ...
    void removeFigure(position_t position) override { state.figures_m[position.x - 1][position.y - 1] = nullptr; }

    BoardState getBoardState() override { return state; }

    void setBoardState() override { return; }
};
