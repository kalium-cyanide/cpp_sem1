...
template <typename type = float> struct Vector2
{
    type x;
    type y;
    ...
    template <typename old_type> static Vector2<type> to_type(std::initializer_list<old_type> list)
    {
...
/* +++ */
template <typename type> inline Vector2<type> operator+ (const Vector2<type>& left, const Vector2<type>& right)
{
    return Vector2<type>(left.x + right.x, left.y + right.y);
}

template <typename type> inline void operator+= (const Vector2<type>& left, const Vector2<type>& right)
{
    left.x += right.x;
    left.y += right.y;
}
...
template <typename type> inline bool operator== (const Vector2<type>& left, const Vector2<type>& right)
{
...
template <typename type> inline std::ostream& operator<< (std::ostream& os, const Vector2<type>& vec)
{
    os << "(" << vec.x << ", " << vec.y << ")";
    return os;
}